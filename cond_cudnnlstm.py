...
def create_model():
    ...
    h1 = Dense(NUM_CELLS, activation='relu')(feature_input)
    h1 = Dense(NUM_CELLS)(h1)
    c1 = Dense(NUM_CELLS, activation='relu')(feature_input)
    c1 = Dense(NUM_CELLS)(c1)
    series_input = Input(shape=(TIME_STEPS, INPUT_DIM))
    lstm0 = CuDNNLSTM(NUM_CELLS, return_sequences=True)(
        series_input, initial_state=[h0, c0])
    lstm_drop = Dropout(DROP_P)(lstm0)
    lstm1 = CuDNNLSTM(NUM_CELLS)(lstm_drop, initial_state=[h1, c1])
...
def create_model_N():
    # construct model
    model_N = Sequential()
    model_N.add(CuDNNLSTM(NUM_CELLS, return_sequences=True,
                          input_shape=(TIME_STEPS, INPUT_DIM)))
    model_N.add(Dropout(DROP_P))
    model_N.add(CuDNNLSTM(NUM_CELLS,
                          input_shape=(TIME_STEPS, INPUT_DIM)))
...
def train_and_test(model, C, y, C_test, y_test, return_epoch=None, **kwargs):
    if return_epoch is not None:
        epoch_save = Pred_History(C_test, return_epoch)
